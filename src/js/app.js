        ...
        proposalsInstance.getNumPatients.call().then(function(numProposals) {
          ...
          for (var i=0; i<numProposals; i++) {
            proposalsInstance.getPatient.call(i).then(function(data) {
              var idx = data[0];
              proposalTemplate.find('.patientNo').text(data[1]);
              proposalTemplate.find('.name').text(data[2]);
              proposalTemplate.find('.gender').text(data[3]);
              proposalTemplate.find('.hospital').text(data[4]);
              proposalTemplate.find('.blood').text(data[5]);
              proposalTemplate.find('.diabetic').text(data[6]);
  ...
  handleAddPatient: function(event) {
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.Medical.deployed().then(function(instance) {
        ...
        return proposalInstance.addPatient(value1, value2, value3, value4, value5, value6, {from: account});
